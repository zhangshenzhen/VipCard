...
public class AdTextViewMult extends LinearLayout {
    List<AdEntity> mTexts; //数据源
    ...
    public AdTextViewMult(Context context) {
        super(context);
        init(context, null);
    }

    public AdTextViewMult(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public AdTextViewMult(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public AdTextViewMult(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
     ...
     */
    public void setTexts(final List<AdEntity> mTexts, int pageSize) {
        this.mTexts = mTexts;
        ...
        for (int i = 0; i < countpage; i++) {
            ...
            for (int j = i * pageSize; j <= limit; j++) {
                View view = inflater.inflate(R.layout.layout_textview_view_item, null);
                TextView tv_mFront = (TextView) view.findViewById(R.id.tv_mFront);
                TextView tv_mBack = (TextView)view.findViewById(R.id.tv_mBack);
                tv_mBack.setText(mTexts.get(j).getmFront());
                tv_mBack.setText(mTexts.get(j).getmBack());
