...
public class ViewPagerCateGridView {
    public ViewPager viewpager_gridview;
    List<NewCategoryBean> list;//HomeTypeBeangen根据项目的需要可自己写
    List<NewCategoryBeanList> listbeans = new ArrayList<>();
    ;//HomeTypeBeanList根据项目的需要可自己写
     ...
     */
    public ViewPagerCateGridView(Context context, ViewPager viewpager_gridview, LinearLayout layout, List<NewCategoryBean> list, int size) {
    ...
    public void initView(List<NewCategoryBean> list, int size) {
     ...
     */
    public void fengzhuang(List<NewCategoryBean> list, int size) {
        ...
        for (int i = 1; i <= num; i++) {
            List<NewCategoryBean> stringList = new ArrayList();
            for (int j = 0; j < list.size(); j++) {
                if (j >= (i - 1) * size && j < i * size) {
                    stringList.add(list.get(j));
                }
            }
            NewCategoryBeanList bean = new NewCategoryBeanList();
...
//        View view = new View(context);
    ...
    class MyPagerAdpater extends PagerAdapter {
        private List<NewCategoryBeanList> listbeans;
        private Context context;


        public MyPagerAdpater(List<NewCategoryBeanList> listbeans, Context context) {
            this.context = context;
    ...
    class MyGridViewAdapter extends BaseAdapter {
        private Context context;
        private List<NewCategoryBeanList> list;
        private int flag;


        public MyGridViewAdapter(Context context, List<NewCategoryBeanList> list, int flag) {
            this.context = context;
            this.list = list;
            this.flag = flag;
        ...
        public View getView(final int position, View convertView, final ViewGroup parent) {
            ...
            viewHolder.textView.setText(list.get(flag).getTypeBeanList().get(position).getCategory_name());
            //AppConfig.DEFAULT_IMG_MERCHANT_BG 配置iamgeloader的显示参数
            ImageLoader.getInstance().displayImage(Config.web.picUrl + list.get(flag).getTypeBeanList().get(position).getCategory_icon(), viewHolder.iv_menu, AppConfig.DEFAULT_IMG_MERCHANT_BG);
    ...
    public interface GridViewClickListener {
        public void onCridViewClickListener(View v, int postion, NewCategoryBean data);
    }
