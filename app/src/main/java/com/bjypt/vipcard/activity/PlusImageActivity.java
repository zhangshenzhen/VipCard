...
import com.bjypt.vipcard.adapter.ViewPagerAdapter;
import com.bjypt.vipcard.dialog.CancelOrOkDialog;
...
public class PlusImageActivity extends Activity implements ViewPager.OnPageChangeListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLook = getIntent().getIntExtra("look", 0);
        initView();
    }
    ...
    private void initView() {
        ...
        mDelete_iv = (ImageView) findViewById(R.id.delete_iv);

        if (mLook == 1) {
            mDelete_iv.setVisibility(View.GONE);
        }

        mDelete_iv.setOnClickListener(this);
    ...
    private void deletePic() {
        CancelOrOkDialog dialog = new CancelOrOkDialog(this, "要删除这张图片吗?") {
            ...
            public void ok() {
                ...
                if (imgList.size() == 0) {
                    mDelete_iv.setEnabled(false);
                } else {
                    mDelete_iv.setEnabled(true);
                }
                setPosition();
                dismiss();
            }
    ...
    private void setPosition() {
        ...
        viewPager.setCurrentItem(mPosition);
        if (imgList.size() == 0) {
            positionTv.setText(mPosition + "/" + imgList.size());
        }
